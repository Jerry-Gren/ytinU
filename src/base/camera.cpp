...
Frustum PerspectiveCamera::getFrustum() const {
    ...
    frustum.planes[Frustum::NearFace] = { pos + znear * fv, fv };
    frustum.planes[Frustum::FarFace] = { pos + frontMultFar, -fv };
    frustum.planes[Frustum::LeftFace] = {
        pos,
        glm::normalize(glm::cross(frontMultFar - rv * halfHSide, uv))
    };
    frustum.planes[Frustum::RightFace] = {
        pos,
        glm::normalize(glm::cross(uv, frontMultFar + rv * halfHSide))
    };
    frustum.planes[Frustum::TopFace] = {
        pos,
        glm::normalize(glm::cross(frontMultFar + uv * halfVSide, rv))
    };
    frustum.planes[Frustum::BottomFace] = {
        pos,
        glm::normalize(glm::cross(rv, frontMultFar - uv * halfVSide))
    };
