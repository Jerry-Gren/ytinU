...
void Application::setFullScreen(bool fullScreen)
{
    ...
    if (fullScreen)
    {
        ...
        // 备份当前窗口的位置和大小，以便以后恢复
        glfwGetWindowPos(_window, &_storedXPos, &_storedYPos);
        glfwGetWindowSize(_window, &_storedWidth, &_storedHeight);

        // 获取主显示器及其视频模式（分辨率、刷新率）
        GLFWmonitor *monitor = glfwGetPrimaryMonitor();
        const GLFWvidmode *mode = glfwGetVideoMode(monitor);

        // 切换到全屏
        // 参数：window, monitor, x, y, width, height, refreshRate
    ...
    else
    {
        ...
        // 恢复到之前备份的位置和大小
        // 将 monitor 设置为 nullptr 即可变回窗口模式
        glfwSetWindowMonitor(_window, nullptr, _storedXPos, _storedYPos, _storedWidth, _storedHeight, 0);
